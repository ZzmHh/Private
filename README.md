...
- Token 鉴权：Agent、5 Agent 运营一键生成、爬虫接口都需要登录后调用
- 运营后台：首个注册用户默认 `isAdmin`；也可在 `.env` 设置 `ADMIN_EMAILS=你的邮箱`（逗号分隔多个），重启后为对应账号开通「运营后台」权限；支持「手动授权套餐」与订单「确认到账并开通」
...
5 Agent 运营一键生成模块可以先调用 Python 爬虫抓取公开页面数据，再交给 OpenClaw 分析。
